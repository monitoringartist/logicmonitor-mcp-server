...
export const costOptimizationToolHandlers: ToolHandlerMap = {
  'list_cost_optimization_recommendations': async ({ client, args }: ToolHandlerContext): Promise<any> => {
  ...
  'get_cost_optimization_recommendation': async ({ client, args }: ToolHandlerContext): Promise<any> => {
    return await client.getCostOptimizationRecommendation(args.id, {
      fields: args.fields,
    });
  },

  'list_cost_optimization_recommendation_categories': async ({ client, args }: ToolHandlerContext): Promise<any> => {
