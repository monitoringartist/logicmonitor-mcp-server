/**
 * SSE / Streamable-HTTP transport runner for the LogicMonitor (LM) MCP server.
 *
 * Extracted from the unified entry point; `index.ts` calls `runHttp` for all
...
export function runHttp(appConfig: ServerConfig, version: string): void {
  ...
  // Trust reverse proxy (Azure Container Apps, Cloudflare, nginx, etc.) so that
  // req.ip and X-Forwarded-For are honored correctly by express-rate-limit and
  // secure-cookie handling. Set EXPRESS_TRUST_PROXY to a number of hops, a CIDR,
  // "true", or leave unset to disable.
  ...
  // application/x-www-form-urlencoded is required for the OAuth 2.1 /oauth/token
  // form body (RFC 6749).
  ...
  if (oauthConfig) {
    ...
    //   5. Server federates to the IdP; on return, /auth/callback
    ...
    // from the authenticated user's IdP group memberships. For now
    // every authenticated user gets the full scope set so that every
    // LM tool is reachable; LM's own RBAC still applies via the
    ...
    app.get('/oauth/authorize', loginLimiter, (req: Request, res: Response, next: NextFunction) => {
      ...
      // Kick off the configured IdP auth — same as /auth/login.
  ...
  // pending authorizations, and expired/rotated refresh tokens).
  if (oauthConfig) {
    startOAuthCleanup();
